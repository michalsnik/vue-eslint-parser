...
class HTMLTransformer {
    ...
    createHTMLText(start, end) {
        const text = this.appendToken("HTMLText", start, end)
        return {
            parent: null,
            type: text.type,
            range: text.range,
    ...
    createHTMLStartTag(node) {
        ...
        result.id = this.createHTMLIdentifier(start + 1)
        ...
        for (const attr of attrs) {
            ...
            const attribute = this.createHTMLAttribute(name, value, attrLoc)
    ...
    createHTMLEndTag(node) {
        ...
        result.id = this.createHTMLIdentifier(start + 2, end - 1)
        result.id.parent = result
    ...
    visitElementNode(node) {
        ...
        result.startTag = this.createHTMLStartTag(node)
        ...
        result.endTag = this.createHTMLEndTag(node)
